...
void ZStream::serialIncoming()
{
  ...
  if(escBufDex>0)
  {
    txFlush(); // keep byte order: anything queued came before the escape sequence
    socketWrite(escBuf,escBufDex);
  }
}

// Serial->socket bytes used to go out one at a time, and with TCP_NODELAY on
// (DEFAULT_NO_DELAY) each byte became its own TCP segment. At 115200 that is
// thousands of one-byte segments a second; lwip's send queue only holds a
// handful, so a sustained or bursty stream filled it, the core's write then
// blocked in ~10 s retry loops and dropped bytes. Queue bytes instead and send
// them as one write: when the buffer is full, when serial has been idle for
// ZSTREAM_TX_IDLE_MS, or when the oldest byte reaches ZSTREAM_TX_MAX_AGE_MS.
void ZStream::txQueue(uint8_t c)
{
  ...
  txBuf[txLen++] = c;
  txLastMs = now;
  if(txLen >= ZSTREAM_TX_BUF_SIZE)
    txFlush();
}

void ZStream::txFlush()
{
  if(txLen == 0)
    return;
  uint8_t len = txLen;
  ...
  if((current != null) && current->isConnected())
    socketWrite(txBuf,len);
}

void ZStream::txFlushIfDue()
{
  if(txLen == 0)
    return;
  unsigned long now = millis();
  if(((now - txLastMs) >= ZSTREAM_TX_IDLE_MS)
  ||((now - txFirstMs) >= ZSTREAM_TX_MAX_AGE_MS))
    txFlush();
}

void ZStream::switchBackToCommandMode(bool pppMode)
{
  txFlush(); // deliver anything still queued if the connection is still up, else drop it
