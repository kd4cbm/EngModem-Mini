...
{
  // A handful of retries with a short delay - SPI-mode SD cards can be slow
  // to respond immediately after a cold power-up, and this runs very early
  // in setup(), before anything else gives the card time to settle.
  bool ok = false;
  for(int attempt=0;(!ok)&&(attempt<5);attempt++)
  {
    if(attempt>0)
      delay(50);
#ifdef DEFAULT_PIN_SD_MOSI
    // SD is on non-default SPI pins on this board (see ESP32S3_PIN_MAP.md) -
    // SD.begin(cs) alone only overrides CS and would otherwise silently fall
    // back to the core's default SPI bus pins, which are already in use here.
    static SPIClass sdSPI(FSPI);
    sdSPI.begin(DEFAULT_PIN_SD_SCK, DEFAULT_PIN_SD_MISO, DEFAULT_PIN_SD_MOSI, DEFAULT_PIN_SD_CS);
    ok = SD.begin(DEFAULT_PIN_SD_CS, sdSPI);
#elif defined(DEFAULT_PIN_SD_CS)
    ok = SD.begin(DEFAULT_PIN_SD_CS);
#else
    ok = SD.begin();
#endif
  }
