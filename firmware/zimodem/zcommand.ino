...
void ZCommand::setOptionsFromSavedConfig(String configArguments[])
{
  ...
  s_pinWrite(pinDCD,dcdStatus);
  // On the EngModem Mini the RTS/CTS GPIOs are fixed by the PCB (GPIO17 is
  // the MAX3237 ROUT2 output, GPIO18 is a MAX3237 DIN3 input), so a saved
  // config written by older firmware - which had the two swapped - must not
  // be allowed to put them back: that would drive GPIO17 against ROUT2 and
  // leave CTS floating. Keep the compiled-in pins, but still re-assert their
  // directions below.
      ...
      case '$':
      {
        ...
        else
        {
          ...
          if(result == ZOK)
          {
            ...
            if(strcmp(var.c_str(),"vfdb")==0)
            {
              // 0=blanked, 1-4=25/50/75/100% - see vfd.ino for why this isn't
              // a finer scale: the hardware only has 4 real brightness levels.
              int level = atoi(val.c_str());
              if((level < 0) || (level > 4) || (!isDigit(val.c_str()[0])))
                result = ZERROR;
              else
                vfdSetBrightnessLevel((uint8_t)level);
            }
#endif
            else
              result = ZERROR;
...
void ZCommand::sendNextPacket()
{
  ...
  while(serial.isSerialOut() && (nextConn != null) && (packetXOn))
  {
    ...
    &&(nextConn != current)) // the connection ATO would resume stays silent until you ATO or ATH it -
                              // genuine Hayes dialup modems don't announce data from a call you +++'d
                              // away from; it just waits quietly. NO CARRIER still fires normally below
                              // if it drops while you're away from it.
