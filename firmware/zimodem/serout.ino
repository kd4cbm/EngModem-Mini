...
FlowControlType ZSerial::flowControlType=DEFAULT_FCT;
...
#       endif
  ...
  else
  {
    ...
    // Once RTS/CTS has been on, the UART peripheral keeps owning the RTS pin and
    // leaves it stuck deasserted after flow control is switched off - which on
    // this board is the PC's CTS line, so a PC honouring CTS would stall. Hand
    // the pin back to plain GPIO and assert it, exactly as at boot.
    if(pinSupport[pinRTS])
    {
      gpio_reset_pin((gpio_num_t)pinRTS);
