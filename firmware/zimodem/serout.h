...
class ZSerial : public Stream
{
  private:
    ...
    // Shared across all ZSerial instances (ZCommand, ZStream, ZConfigMode,
    // etc. each keep their own ZSerial member), because setFlowControlType()
    // always reconfigures the one real physical UART peripheral regardless
    // of which instance calls it - a per-instance copy of this value goes
    // stale the moment a different mode's ZSerial changes it (e.g. the VFD's
    // status line reads ZStream's copy, which ATF0/AT&K3 - issued through
    // ZCommand's own copy - never touched).
    static FlowControlType flowControlType;
    bool XON_STATE=true;
